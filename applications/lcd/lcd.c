...
//LCD的画笔颜色和背景色
rt_uint32_t POINT_COLOR=0xFF000000;     //画笔颜色
rt_uint32_t BACK_COLOR =0xFFFFFFFF;     //背景色
...
//x,y:坐标
//返回值:此点的颜色
rt_uint32_t LCD_ReadPoint(rt_uint16_t x,rt_uint16_t y)
{
...
//x,y:坐标
//color:颜色
void LCD_Fast_DrawPoint(rt_uint16_t x,rt_uint16_t y,rt_uint32_t color)
{
    if(lcddev.id==0X1963)
    {
...
void LCD_Init(void)
{
    ...
    LOG_D("lcd id : %02x",lcddev.id);
    if(lcddev.id!=0X9341)       //·Ç9341,³¢ÊÔ¿´¿´ÊÇ²»ÊÇNT35310
    {
        LCD_WR_REG(0XD4);
        lcddev.id=LCD_RD_DATA();//dummy read
    ...
    if(lcddev.id==0X9341)   //9341³õÊ¼»¯
        ...
        //³õÊ¼»¯Íê³ÉÒÔºó,ÌáËÙ
        if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//Èç¹ûÊÇÕâ¼¸¸öIC,ÔòÉèÖÃWRÊ±ÐòÎª×î¿ì
        {
